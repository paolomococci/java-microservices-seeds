...
public class ItemEditorView
    ...
    public ItemEditorView() {
        ...
        this.itemGrid.asSingleSelect().addValueChangeListener(listener -> {
            if (listener.getValue() != null) {
                Optional<Item> itemFromBackend = Optional.ofNullable(
                        this.itemRestfulController.read(
                                listener.getValue().get_links().getSelf().getHref()
                        ));
                if (itemFromBackend.isPresent()) {
                    this.populate(itemFromBackend.get());
                } else {
                    this.refresh();
                }
            } else {
        ...
        this.update.addClickListener(listener -> {
            try {
                if (this.item != null) {
                    ...
                    this.itemRestfulController.update(
                            this.item,
        ...
        this.create.addClickListener(listener -> {
            try {
                if (!this.code.getValue().isEmpty()) {
                    ...
                    this.itemBinder.writeBean(this.item);
                    this.itemRestfulController.create(
                            this.item
                    );
                    this.clear();
                    this.refresh();
        ...
        this.delete.addClickListener(listener -> {
            try {
                if (this.item != null) {
                    this.itemBinder.writeBean(this.item);
                    this.itemRestfulController.delete(
                            this.item.get_links().getSelf().getHref()
                    );
                    this.clear();
                    this.refresh();
    ...
    private void reload() {
        this.itemGrid.setItems(itemRestfulController.readAll());
    }
